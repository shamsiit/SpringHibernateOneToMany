	...
	<table style="border: 1px solid; width: 100%; text-align: center">
		...
		<tbody style="background: #ccc">
			<c:forEach items="${persons}" var="person">
				...
				<c:url var="deleteUrl"
					value="/main/record/delete?id=${person.id}" />
				...
				<c:if test="${!empty person.creditCards}">
					<c:forEach items="${person.creditCards}" var="creditCard">
						<tr>
							...
							<c:url var="addCcUrl"
								value="/main/creditcard/add?id=${person.id}" />
							<c:url var="editCcUrl"
								value="/main/creditcard/edit?pid=${person.id}&cid=${creditCard.id}" />
							<c:url var="deleteCcUrl"
								value="/main/creditcard/delete?id=${creditCard.id}" />
